...
public class ConetModule implements IFloodlightModule, IOFMessageListener, MsgTransportListener
{
    ...
    /** Default datapath */
    // NOTE: this should be removed when the cache server will send datapath within cache-to-controller messages 
    ...
    public String sw_virtual_mac_addr="000000000001";



    /** DEBUG: whether disabling the redirection to cache-server of data packets coming from icn-servers */
    public boolean debug_disable_redirection=false;

    /** DEBUG: whether disabling any static flow table entries and working only in transparent learning switch mode */
    public boolean debug_learning_switch_only=false;


    /** Switches. */
    Hashtable<Long,IOFSwitch> switches=new Hashtable();

    /** Mapping of Conet IP addresses (String) to SW ports (Short), for each SW datapath (Long). */
    ...
    /** Cached contents (HashSet<String>) */
    Hashtable<String,CachedContent> cached_contents=new Hashtable<String,CachedContent>();
    ...
    /** Deletes all flowtable entries for cached contents and clears the DB of cached contents. */
    public void flushAllContents()
    {   if (DEFAULT_DATAPATH>=0) flushAllContents(DEFAULT_DATAPATH);
        else println("DEBUG: No default datapath has been set: impossible to delete contents");
    } 
    ...
    {   println("DELETE ALL CONTENTS");
        ...
        {   String content_name=(String)i.nextElement();
            long tag=Long.parseLong(content_name);
            // remove flowtable entries that match packets with the given tag and with any icn server as destination
            // NOTE: this should be changed when the cache server will send also the destination (together with tag info) within cache-to-controller messages 
            for (int j=0; j<conet_servers.length; j++)
                redirectToCache(datapath,OFFlowMod.OFPFC_DELETE,(int)BinTools.fourBytesToInt(BinAddrTools.ipv4addrToBytes(conet_servers[j])),(byte)conet_proto,tag);
        }
    ...
    {   for (int i=0; i<sw_datapath.length; i++)
           if (sw_datapath_long[i]==datapath) return cache_ip_addr[i];
           //else println("DEBUG: getCacheIpAddress(): "+datapath+"!="+sw_datapath_long[i]+"("+Long.toHexString(sw_datapath_long[i])+")");
        return null;
    }   
    ...
    {   for (int i=0; i<sw_datapath.length; i++)
           if (sw_datapath_long[i]==datapath) return cache_mac_addr[i];
        return null;
    }   
    ...
    {   for (int i=0; i<sw_datapath.length; i++)
           if (sw_datapath_long[i]==datapath) return (short)cache_port[i];
        return 0;
    }   
    ...
    {   if (conet_port_mapping.containsKey(datapath))
        {  Short port=conet_port_mapping.get(datapath).get(node_ipaddr);
           if (port!=null) return port.shortValue();
        }
        return 0;
    }   
    ...
    {   if (!conet_port_mapping.containsKey(datapath)) conet_port_mapping.put(datapath,new Hashtable());
    ...
    {
        if (LEARNING_SWITCH_VERBOSE) println("receive(): "+msg);
        ...
        {   if (debug_learning_switch_only || !tag_based_forwarding) return processPacketInMessage(sw,(OFPacketIn)msg,cntx);
            else return processConetPacketInMessage(sw,(OFPacketIn)msg,cntx);
        }
        // else
        if (msg.getType()==OFType.PORT_STATUS) return processPortStatusMessage(sw,(OFPortStatus)msg);
        // else
        if (msg.getType()==OFType.FLOW_REMOVED) return processFlowRemovedMessage(sw,(OFFlowRemoved)msg);
        // else
        if (msg.getType()==OFType.HELLO) return processHelloMessage(sw,(OFHello)msg);
        // else
        if (msg.getType()==OFType.ERROR)
        {   if (LEARNING_SWITCH_VERBOSE) println("LearningSwitch: receive(): received from switch "+sw+" the error: "+msg);
            return Command.CONTINUE;
        }
        // else
        if (LEARNING_SWITCH_VERBOSE) println("LearningSwitch: receive(): received from switch "+sw+" the unexpected msg: "+msg);
        return Command.CONTINUE;
    ...
    public void init(FloodlightModuleContext context) throws FloodlightModuleException
    {
        ...
        // LOG
        if (log==null)
        {  log=new RotatingLog(log_file,1,(((long)log_size)*1024)*1024,log_rotation,RotatingLog.DAY,log_time);
           log.setTimestamp(true);
        }
        
        // in case a colon-formatted mac address has been given 
        for (int i=0; i<cache_mac_addr.length; i++) cache_mac_addr[i]=BinAddrTools.trimHexString(cache_mac_addr[i]);
        
        // in case a colon-formatted sw_datapath address has been given 
        for (int i=0; i<sw_datapath.length; i++) sw_datapath[i]=BinAddrTools.trimHexString(sw_datapath[i]);
        // convert (String)sw_datapath to (long)sw_datapath_long
        sw_datapath_long=new long[sw_datapath.length];
        for (int i=0; i<sw_datapath.length; i++) sw_datapath_long[i]=Long.parseLong(sw_datapath[i],16);
        for (int i=0; i<sw_datapath.length; i++) println("sw_datapath["+i+"]="+sw_datapath_long[i]+"("+Long.toHexString(sw_datapath_long[i])+")");
        ...
        try
        {  // start json interface server for cache-to-controller communication
    ...
    public void startUp(FloodlightModuleContext context)
    {
        ...
        println("");
        println("startUp(): add Conent REST API");
        println("");
        ...
        //restApi.addRestletRoutable(new net.floodlightcontroller.core.web.CoreWebRoutable());
    }
        ...
        {   JSONObject json_message=(JSONObject)JSONValue.parse(msg.toString());
            ...
            {  if (json_message.containsKey("MAC"))
               {  String json_cache_mac_addr=BinAddrTools.trimHexString(json_message.get("MAC").toString());
                  if (!json_cache_mac_addr.equals(getCacheMacAddress(DEFAULT_DATAPATH))) println("WARNING: cache MAC address missmatch!");
               }
               if (json_message.containsKey("IP"))
               {  String json_cache_ip_addr=json_message.get("IP").toString();
                   if (!json_cache_ip_addr.equals(getCacheIpAddress(DEFAULT_DATAPATH))) println("WARNING: cache IP address missmatch!");
               }
               long datapath=DEFAULT_DATAPATH;
               if (json_message.containsKey("DataPath")) datapath=Long.parseLong(json_message.get("DataPath").toString());
               // reset cached contents
               flushAllContents(datapath);
               /*for (Enumeration i=cached_contents.keys(); i.hasMoreElement(); )
               {   String content_name=(String)i.nextElement();
                   long tag=Long.parseLong(content_name);
                   // remove flowtable entries that match packets with the given tag and with any icn server as destination
                   // NOTE: this should be changed when the cache server will send also the destination (together with tag info) within cache-to-controller messages 
                   for (int j=0; j<conet_servers.length; j++)
                       redirect(datapath,OFFlowMod.OFPFC_DELETE,(int)BinTools.fourBytesToInt(BinAddrTools.ipv4addrToBytes(conet_servers[j])),(byte)conet_proto,tag);
               }
               cached_contents.clear();*/
            }
            else
            if (tag_based_forwarding && (type.equalsIgnoreCase("stored") || type.equalsIgnoreCase("refreshed") || type.equalsIgnoreCase("deleted")))
            {  String content_name=json_message.get("CONTENT NAME").toString();
               long tag=Long.parseLong(content_name);
               String nid=null;
               if (json_message.containsKey("nid")) nid=json_message.get("nid").toString();
               long csn=-1;
               if (json_message.containsKey("csn")) csn=Long.parseLong(json_message.get("csn").toString());
               CachedContent content=new CachedContent(nid,csn,tag);
               
               long datapath=DEFAULT_DATAPATH;
               if (json_message.containsKey("DataPath")) datapath=Long.parseLong(json_message.get("DataPath").toString());
               int dest_ipaddr=0;
               if (json_message.containsKey("DestIpAddr")) dest_ipaddr=(int)BinTools.fourBytesToInt(BinAddrTools.ipv4addrToBytes(json_message.get("DestIpAddr").toString()));
               
               short command=-1;
               if (type.equalsIgnoreCase("stored"))
               {  cached_contents.put(content_name,content);
                  command=OFFlowMod.OFPFC_ADD;
               }
               else
               if (type.equalsIgnoreCase("refreshed"))
               {  if (!cached_contents.containsKey(content_name)) cached_contents.put(content_name,content);
                  command=OFFlowMod.OFPFC_ADD;
               }
               else
               if (type.equalsIgnoreCase("deleted"))
               {  cached_contents.remove(content_name);
                  command=OFFlowMod.OFPFC_DELETE;
               }
               // set flowtable entry for redirecting packets with the given tag and with the given destination
               if (dest_ipaddr!=0) redirectToCache(datapath,command,dest_ipaddr,(byte)conet_proto,tag);
               else
               // set flowtable entry for redirecting packets with the given tag and with any conet server address as destination
               // NOTE: this should be chnged when the cache server will send also the destination (together with tag info) within cache-to-controller messages 
               {  for (int i=0; i<conet_servers.length; i++)
                      redirectToCache(datapath,command,(int)BinTools.fourBytesToInt(BinAddrTools.ipv4addrToBytes(conet_servers[i])),(byte)conet_proto,tag);
               }
            }
    ...
    {   println("Transport "+transport+" terminated.");
        if (error!=null) println("Transport "+transport+" terminated due to: "+error.toString());
    }
    ...
    {   
        ...
        {  OFActionNetworkLayerDestination action_nw_dest=new OFActionNetworkLayerDestination();
           action_nw_dest.setNetworkAddress((int)BinTools.fourBytesToInt(BinAddrTools.ipv4addrToBytes(getCacheIpAddress(datapath))));
        ...
        {  OFActionDataLayerSource action_dl_src=new OFActionDataLayerSource();
           action_dl_src.setDataLayerAddress(BinTools.hexStringToBytes(sw_virtual_mac_addr));
           actions_vector.addElement(action_dl_src);
           actions_len+=OFActionDataLayerSource.MINIMUM_LENGTH;
        }   
        //short port_out=(command!=OFFlowMod.OFPFC_DELETE)? getCachePort(datapath) : OFPort.OFPP_NONE.getValue();         
        //OFActionOutput action_output=new OFActionOutput(port_out,(short)0xffff);
        ...
        else println("WARNING: redirect(): No switch found for datapath "+datapath+".");
    }
    ...
    {   // SEND TO ICN-CLIENT AND EVENTUALLY TO CACHE-SERVER
        ...
        { // SEND ONLY TO ICN-CLIENT IF COMING FROM CACHE-SERVER, OTHERWISE SEND TO BOTH ICN-CLIENT AND TO CACHE-SERVER
          ...
          // Date: 15/1/2013
          // Reason: the static rule was deleted when other dynamic rules (e.g. for ARP packet in opposite direction) exired
          // Changes:
          //doFlowModStatic(switches.get(dp),command,(short)(PRIORITY_STATIC+1),(short)0,vlan,(short)0x800,BinTools.hexStringToBytes(getCacheMacAddress(dp)),0,client_macaddr,client_ipaddr,(byte)conet_proto,(short)0,(short)0,port);
          doFlowModStatic(switches.get(dp),command,(short)(PRIORITY_STATIC+1),(short)0,vlan,(short)0x800,null,(int)BinTools.fourBytesToInt(BinAddrTools.ipv4addrToBytes(getCacheIpAddress(dp))),null,client_ipaddr,(byte)conet_proto,(short)0,(short)0,port);
          ...
          if (change_destination)
          {   OFActionNetworkLayerDestination action_nw_dest=new OFActionNetworkLayerDestination();
              action_nw_dest.setNetworkAddress((int)BinTools.fourBytesToInt(BinAddrTools.ipv4addrToBytes(getCacheIpAddress(dp))));
              actions_vector.addElement(action_nw_dest);
              actions_len+=OFActionNetworkLayerDestination.MINIMUM_LENGTH;
          }           
          if (change_mac_source)
          ...
          //@@@@@@
          //doFlowModStatic(switches.get(dp),OFFlowMod.OFPFC_ADD,PRIORITY_STATIC,(short)0,vlan,eth_proto,null,0,eth_src,(int)ip_src,(byte)conet_proto,(short)0,(short)0,Arrays.asList(actions),((short)actions_len));
          doFlowModStatic(switches.get(dp),command,PRIORITY_STATIC,(short)0,vlan,(short)0x800,null,0,client_macaddr,client_ipaddr,(byte)conet_proto,(short)0,(short)0,actions_vector,((short)actions_len));
        }
        else
        {  // SEND ONLY TO ICN-CLIENT
           //doFlowModStatic(switches.get(dp),OFFlowMod.OFPFC_ADD,PRIORITY_STATIC,(short)0,vlan,eth_proto,null,0,eth_src,(int)ip_src,(byte)conet_proto,(short)0,(short)0,port_in);
           doFlowModStatic(switches.get(dp),command,PRIORITY_STATIC,(short)0,vlan,(short)0x800,null,0,client_macaddr,client_ipaddr,(byte)conet_proto,(short)0,(short)0,port);
        }
    }
    ...
    {   // SEND TO ICN-SERVER
        //@@@@@@
        //doFlowModStatic(switches.get(dp),OFFlowMod.OFPFC_ADD,(short)(PRIORITY_STATIC+1),(short)0,vlan,eth_proto,null,0,eth_src,(int)ip_src,(byte)conet_proto,(short)0,(short)0,(short)port_in);
        doFlowModStatic(switches.get(dp),command,(short)(PRIORITY_STATIC+1),(short)0,vlan,(short)0x800,null,0,server_macaddr,server_ipaddr,(byte)conet_proto,(short)0,(short)0,port);
    }
    ...
    protected Command processHelloMessage(IOFSwitch sw, OFHello hello)
    {   // Do nothing; it can be removed.
        return Command.CONTINUE;
    }


    /** Processes PacketIn message Conet-aware. */
    ...
    {  
       ...
       {  
         ...
         if (!verbose) verbose=eth_proto==0x800 && ip_proto==conet_proto;
         if (verbose && !LEARNING_SWITCH_VERBOSE)
         {  verbose=false;
            long sw_id=sw.getId();
            for (int i=0; !verbose && i<sw_datapath_long.length; i++) if (sw_datapath_long[i]==sw_id) verbose=true;
         }

         if (verbose) println("PACKET-IN: dp="+sw.getId()+"("+Long.toHexString(sw.getId())+"), port="+port_in+", vlan="+vlan+", mac_src="+BinTools.asHex(eth_src)+", mac_dst="+BinTools.asHex(eth_dst)+", eth_proto=0x"+Integer.toHexString(U16.f(eth_proto))+", ip_src="+BinAddrTools.bytesToIpv4addr(BinTools.intTo4Bytes(ip_src))+", ip_dst="+BinAddrTools.bytesToIpv4addr(BinTools.intTo4Bytes(ip_dst))+", ip_proto=0x"+Integer.toHexString(ip_proto)+", tag=0x"+Long.toHexString(tag));

         IpPacket ip_packet=(eth_proto==0x800)? IpPacket.parseRawPacket(pkt_data,14) : null;
         //println("DEBUG: "+((ip_packet!=null)? "is" : "is NOT")+" an IP packet");
         //if (ip_packet!=null)
         ...
         //}
         boolean is_conet=(ip_packet!=null && ip_packet.hasOptions())? (BinTools.uByte(ip_packet.getOptionsBuffer()[ip_packet.getOptionsOffset()])==ConetHeader.IP4_OPT_TYPE_CONET) : false;
         ...
         {   if (CONET_VERBOSE) println("DEBUG: no cache server found for this datapath "+Long.toHexString(dp.longValue())+". nothing to do.");
         }
         else
         {         
            ...
            {   if (verbose) println("CHECK CACHE SERVER: ip_addr="+cache_ipaddr+", mac_addr="+BinTools.asHex(eth_src)+", port="+port_in);
            }
   
            // LEARN MAC AND PORT OF ICN CLIENTS
            for (int i=0; conet_clients!=null && i<conet_clients.length; i++)
            {  if (getConetNodePort(dp,conet_clients[i])>0) continue;
               // else            
               int conet_ip_addr=(int)BinTools.fourBytesToInt(BinAddrTools.ipv4addrToBytes(conet_clients[i]));
               if (ip_src==conet_ip_addr)
               {   println("LEARN CONET CLIENT: eth_proto="+Integer.toString(eth_proto,16)+",ip_src="+BinAddrTools.bytesToIpv4addr(BinTools.intTo4Bytes(ip_src))+", port_in="+port_in);
            ...
            {  if (getConetNodePort(dp,conet_servers[i])>0) continue;
               // else            
               int conet_ip_addr=(int)BinTools.fourBytesToInt(BinAddrTools.ipv4addrToBytes(conet_servers[i]));
               if (ip_src==conet_ip_addr)
               {   println("LEARN CONET SERVER: eth_proto="+Integer.toString(eth_proto,16)+",ip_src="+BinAddrTools.bytesToIpv4addr(BinTools.intTo4Bytes(ip_src))+", port_in="+port_in);
         ...
         {   println("ARP PACKET: ");
             ...
             learnFromSourceMac(sw,(short)port_in,(Long)Ethernet.toLong(eth_src),(Short)eth_vlan);
             // forward packet
         ...
         // OTHER PACKETS
         {   println("IP PACKET:");
             String ip_src=BinAddrTools.bytesToIpv4addr(BinTools.intTo4Bytes(match.getNetworkSource()));
             String ip_dst=BinAddrTools.bytesToIpv4addr(BinTools.intTo4Bytes(match.getNetworkDestination()));
             int tp_src=U16.f(match.getTransportSource());
             int tp_dst=U16.f(match.getTransportDestination());

             println("    Switch: "+sw.getId());
             println("    Ingress port: "+port_in);
             println("    ETH: "+BinTools.asHex(eth_src)+" -> "+BinTools.asHex(eth_dst)+" [proto=0x"+Integer.toHexString(eth_proto)+"]");
             println("    IP: "+ip_src+":"+tp_src+" -> "+ip_dst+":"+tp_dst+" [proto="+ip_proto+"]");
       ...
       if (verbose) println("PACKET-IN: END\n");
       return command;
    }
    ...
    protected void removedSwitch(IOFSwitch sw) {
    ...
    protected Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) {
        ...
        if ((destMac & 0xfffffffffff0L) == 0x0180c2000000L) {
            if (LEARNING_SWITCH_VERBOSE) println("LearningSwitch: processPacketInMessage(): ignoring packet addressed to 802.1D/Q reserved addr: switch "+sw+" vlan "+vlan+" dest MAC "+Long.toString(destMac,16));
            return Command.STOP;
        }

        boolean verbose=CONET_VERBOSE || (match.getDataLayerType()==0x800 && match.getNetworkProtocol()==conet_proto);
        if (verbose && !LEARNING_SWITCH_VERBOSE)
        {   verbose=false;
            long sw_id=sw.getId();
            for (int i=0; !verbose && i<sw_datapath_long.length; i++) if (sw_datapath_long[i]==sw_id) verbose=true;
        ...
        if (outPort == null) {
            ...
            //     a lot of floods.
            if (verbose) println("DEBUG: packet out to all ports");
            doPacketOutForPacketIn(sw, pi, OFPort.OFPP_FLOOD.getValue());
        }
        else
        if (outPort == match.getInputPort())
        {   if (verbose) println("DEBUG: port_in == port_out: packet ignored");
            if (verbose && LEARNING_SWITCH_VERBOSE) println("LearningSwitch: processPacketInMessage(): ignoring packet that arrived on same port as learned destination:"
        ...
        {   // Add flow table entry matching source MAC, dest MAC, VLAN and input port
            ...
            if (verbose) println("DEBUG: packet out to port "+outPort+" (and flow add)");
    ...
    protected Command processPortStatusMessage(IOFSwitch sw, OFPortStatus portStatusMessage) {
        ...
        if (LEARNING_SWITCH_VERBOSE) println("LearningSwitch: processPortStatusMessage(): received port status: " + portStatusMessage.getReason() + " for port " + port.getPortNumber());
    ...
    protected Command processFlowRemovedMessage(IOFSwitch sw, OFFlowRemoved flowRemovedMessage) {
        ...
        if (LEARNING_SWITCH_VERBOSE) println("LearningSwitch: processFlowRemovedMessage(): "+sw+" flow entry removed "+flowRemovedMessage);
        ...
        removeFromPortMap(sw, Ethernet.toLong(match.getDataLayerSource()),
        ...
        doFlowMod(sw, OFFlowMod.OFPFC_DELETE, -1, match.clone()
    ...
    protected void doPacketOut(IOFSwitch sw, short port_in, int buffer_id, byte[] pkt_data, short port_out, FloodlightContext cntx)
    {
    ...
    {   // read in packet data headers by using OFMatch
        ...
        for (int i=0; i<actions.length; i++) actions[i]=new OFActionOutput(ports_out[i],(short)0xffff);
        doFlowMod(sw,OFFlowMod.OFPFC_ADD,pi.getBufferId(),match,Arrays.asList(actions),((short)actions.length*OFActionOutput.MINIMUM_LENGTH));
    }
    ...
    {   // Read in packet data headers by using OFMatch
        ...
        {   doFlowMod(sw, OFFlowMod.OFPFC_ADD, -1, match.clone()
                 .setDataLayerSource(match.getDataLayerDestination())
                 .setDataLayerDestination(match.getDataLayerSource())
                 .setNetworkSource(match.getNetworkDestination())
                 .setNetworkDestination(match.getNetworkSource())
    ...
    protected void doFlowMod(IOFSwitch sw, short command, int bufferId, OFMatch match, List<OFAction> actions, int actions_len) {           
        ...
        if (LEARNING_SWITCH_VERBOSE) println("LearningSwitch: doFlowMod(): "+sw+" "+((command == OFFlowMod.OFPFC_DELETE) ? "deleting" : "adding")+" flow mod "+flowMod);
    ...
    protected void doFlowModStatic(IOFSwitch sw, short command, short priority, short port_in, short vlan, short eth_proto, byte[] src_macaddr, int src_ipaddr, byte[] dest_macaddr, int dest_ipaddr, byte ip_proto, long tag, short port_out)     
    {   long src_tport=(tag>>16)&0xFFFF;
        long dest_tport=tag&0xFFFF;
        doFlowModStatic(sw,command,priority,port_in,vlan,eth_proto,src_macaddr,src_ipaddr,dest_macaddr,dest_ipaddr,ip_proto,(short)src_tport,(short)dest_tport,port_out);
    }


    /** Modifies (adds or deletes) a static flow entry. */
    protected void doFlowModStatic(IOFSwitch sw, short command, short priority, short port_in, short vlan, short eth_proto, byte[] src_macaddr, int src_ipaddr, byte[] dest_macaddr, int dest_ipaddr, byte ip_proto, short src_tport, short dest_tport, short port_out)     
    {   //if (command==OFFlowMod.OFPFC_DELETE) port_out=OFPort.OFPP_NONE.getValue();
        List<OFAction> actions=Arrays.asList((OFAction)new OFActionOutput(port_out,(short)0xffff));
        int actions_len=OFActionOutput.MINIMUM_LENGTH;
        doFlowModStatic(sw,command,priority,port_in,vlan,eth_proto,src_macaddr,src_ipaddr,dest_macaddr,dest_ipaddr,ip_proto,src_tport,dest_tport,actions,actions_len);
    }


    /** Modifies (adds or deletes) a static flow entry. */
    protected void doFlowModStatic(IOFSwitch sw, short command, short priority, short port_in, short vlan, short eth_proto, byte[] src_macaddr, int src_ipaddr, byte[] dest_macaddr, int dest_ipaddr, byte ip_proto, long tag, List<OFAction> actions, int actions_len)     
    {   long src_tport=(tag>>16)&0xFFFF;
        long dest_tport=tag&0xFFFF;
        doFlowModStatic(sw,command,priority,port_in,vlan,eth_proto,src_macaddr,src_ipaddr,dest_macaddr,dest_ipaddr,ip_proto,(short)src_tport,(short)dest_tport,actions,actions_len);
    }


    /** Modifies (adds or deletes) a static flow entry. */
    protected void doFlowModStatic(IOFSwitch sw, short command, short priority, short port_in, short vlan, short eth_proto, byte[] src_macaddr, int src_ipaddr, byte[] dest_macaddr, int dest_ipaddr, byte ip_proto, short src_tport, short dest_tport, List<OFAction> actions, int actions_len)     
    {   OFMatch match=new OFMatch();
        ...
        println();
        println("doStaticFlowAdd(): "+sw+" "+((command==OFFlowMod.OFPFC_DELETE)? "DELETE": "ADD")+" static flow entry "+flowMod);
        println();
        ...
        {   if (LEARNING_SWITCH_VERBOSE) println("LearningSwitch: doFlowModStatic(): ERROR: Failed to write "+flowMod+" to switch "+sw+": "+e);
        }
    }
    ...
    {   println("DEBUG: DELETE ALL STATIC FLOW ENTRIES (FOR ALL SWITCHES)");
        for (Enumeration<IOFSwitch> e=switches.elements(); e.hasMoreElements(); )
    ...
    {   println();
        ...
        doFlowModStatic(sw,OFFlowMod.OFPFC_DELETE,(short)0,(short)0,vlan,(short)0x800,null,(int)0,null,(int)0,(byte)0,(short)0,(short)0,null,0);
    }
    ...
    {   println("DEBUG: INSERTS SOME CONTENTS FOR TESTING");
        cached_contents.put("869728095",new CachedContent("_example_1k3",0,869728095));
        cached_contents.put("869728096",new CachedContent("_example_1k3",1,869728096));
        cached_contents.put("869728097",new CachedContent("_example_1k3",2,869728097));
        cached_contents.put("123456789",new CachedContent("_abc_def_ghi_jkl",123,123456789));
    }
    ...
    {   println("DELETE ALL FLOWTABLE STATIC ENTRIES (METHOD 2)");
        ...
        OFFlowMod flowMod=(OFFlowMod)floodlightProvider.getOFMessageFactory().getMessage(OFType.FLOW_MOD);
        //match.setWildcards(~(OFMatch.OFPFW_DL_TYPE | OFMatch.OFPFW_NW_PROTO | OFMatch.OFPFW_TP_DST));   
        ...
        {   if ( ofSwitch==null)
            {   println("DEBUG: Switch is not connected!");
                continue;
            }
            try
            ...
            {   println("DEBUG: tried to write flow_mod delete to "+ofSwitch.getId()+" but failed: "+e.getMessage());
            }
        }
    }
    ...
    /*protected void testFlowModStatic(IOFSwitch sw)
    {   if (static_rules_done) return;
        // else
        println("DEBUG: setStaticFlowRules() of sw 0x"+Long.toHexString(sw.getId()));
        ...
        {  static_rules_done=true;
           ...
           //int src_ipaddr=(int)BinTools.fourBytesToInt(BinAddrTools.ipv4addrToBytes("192.168.1.23"));
           int src_ipaddr=(int)0;
           byte[] dest_macaddr=BinTools.hexStringToBytes(BinAddrTools.trimHexString("02:03:00:00:00:b9"));
           int dest_ipaddr=(int)BinTools.fourBytesToInt(BinAddrTools.ipv4addrToBytes("192.168.1.8"));
           byte ip_proto=(byte)17;
           short src_tport=(short)2048;
           short dest_tport=(short)2048;
           short port_out=(short)12;

           short commad=OFFlowMod.OFPFC_ADD;
           short priority=PRIORITY_REDIRECTION;

           doFlowModStatic(sw,commad,priority,port_in,vlan,eth_proto,src_macaddr,src_ipaddr,dest_macaddr,dest_ipaddr,ip_proto,src_tport,dest_tport,port_out);
        }
    }*/
